...
plt.ion() # to enable interactive drawing


class springPlotter:
    '''
        This class plots the time histories for the spring mass system data
    '''
    ...
    def __init__(self):
        ...
        # Respresenation of lists to hold the time and data histories
        self.time_history = [] # time
        self.zref_history = [] #reference position z_ref
        self.z_history = [] # position z
        self.Force_history = [] # control force

        # create a handle for every subplot
        self.handle = []
        self.handle.append(myPlot(self.ax[0],ylabel='z(m)',title='Mass spring system data'))
        self.handle.append(myPlot(self.ax[1],xlabel='t(s)',ylabel='Force(N)'))

    def update(self,t,reference,states,ctrl):
        '''
            Add to time and data histories and update the plots
        '''
        # update the time history of all the plot variables
...
class myPlot:
    ...
    def update(self, time, data):
        ...
        '''
        if self.init == True:  # Initialize the plot when the first time routine is called
